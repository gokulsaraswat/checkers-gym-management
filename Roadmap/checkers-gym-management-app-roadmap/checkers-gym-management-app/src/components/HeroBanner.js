...
const HeroBanner = () => {
  ...
  const openDashboardPath = profile?.role === 'admin' ? '/admin' : '/dashboard';
    ...
    >
      ...
      <Stack direction={{ xs: 'column', sm: 'row' }} gap={2} mt={4}>
        <Button
          ...
          to={user ? openDashboardPath : '/auth'}
          variant="contained"
          sx={{
        ...
        >
          {user ? (profile?.role === 'admin' ? 'Open Admin Panel' : 'Open Dashboard') : 'Login / Signup'}
        </Button>
        <Button
